package com.example.vincenttext;
...
public class MessageAdapter extends RecyclerView.Adapter<MessageAdapter.MessageViewHolder> {
    ...
    public void onBindViewHolder(@NonNull final MessageViewHolder messageViewHolder, final int position)
    {
        ...
        else if(fromMessageType.equals("image"))
        {
            ...
            if(fromUserID.equals (messageSenderId))
            {
                ...
                messageViewHolder.messageSenderPicture.setBackgroundResource (R.drawable.pfd_file);
            }
            else{
                messageViewHolder.receiverProfileImage.setVisibility (View.VISIBLE);
                messageViewHolder.messageReceiverPicture.setVisibility (View.VISIBLE);

                messageViewHolder.messageReceiverPicture.setBackgroundResource (R.drawable.pfd_file);
    ...
    private void deleteSentMessages(final int position, final MessageViewHolder holder)
    {
        ...
        rootRef.child ("Messages").child(userMessagesList.get(position).getFrom ()).child(userMessagesList.get(position).getTo()).child(userMessagesList
        .get(position).getMessageID()).removeValue ().addOnCompleteListener (new OnCompleteListener<Void> () {
            @Override
            public void onComplete (@NonNull Task<Void> task) {
                if(task.isSuccessful ())
                {
                    ...
                    Toast.makeText (holder.itemView.getContext (), "An Error Occured...", Toast.LENGTH_SHORT).show ();
                }
            }
        });
    ...
    private void deleteReceiveMessages(final int position, final MessageViewHolder holder)
    {
        ...
        rootRef.child ("Messages").child(userMessagesList.get(position).getTo ()).
                child(userMessagesList.get(position).getFrom ()).child(userMessagesList
                .get(position).getMessageID()).removeValue ().addOnCompleteListener (new OnCompleteListener<Void> () {
            @Override
            public void onComplete (@NonNull Task<Void> task) {
                if(task.isSuccessful ())
